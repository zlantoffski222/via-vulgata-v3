    ...
    <div className="page fade-in" style={{ maxWidth: 760 }}>
      ...
      <div className="card" style={{ marginTop: 16 }}>
        ...
        <div className="togglelist">
          <Toggle on={!!s.simple} set={v => { settingsStore.set(v ? { simple: true, mode: 'en', enVersion: 'cpdv', showXrefs: false, showOriginal: false, showFathers: false, showGlosses: true, greatStories: true, fontSize: Math.max(s.fontSize, 20) } : { simple: false, mode: 'parallel', enVersion: 'drc', showXrefs: true, greatStories: false }); }} label="Simple mode — for children and first-timers" help="English only, in today's words (the Catholic Public Domain Version), larger type, no cross-references or Greek, and the Story trimmed to the great stories. Turn it off to have everything back" />
          <Toggle on={s.font === 'sans'} set={v => settingsStore.set({ font: v ? 'sans' : 'serif' })} label="Easier-to-read type" help="A plain sans-serif with wider spacing for the verses, which many readers with dyslexia find easier than the book face" />
          ...
          <Toggle on={!!s.showFathers} set={v => settingsStore.set({ showFathers: v })} label="A word from the Fathers" help="A ✝ mark on the verses the early Church wrote about — Augustine, Jerome, Chrysostom, Ambrose, Gregory, Irenaeus — tap for a line of theirs. Off by default so the text speaks first" />
